...
<body>
    ...
    if (isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true) {
            ...
            $query = "SELECT eventID, eventTitle, eventType, eventDescription, eventStart, eventEnd, numAttendees 
                      FROM tbl_events WHERE stateID = 2";
...
<tbody id="eventTableBody">
    ...
    function deleteExpiredEvents($link) {
        ...
        $query = "DELETE FROM tbl_events WHERE eventEnd < ?";
    
        if ($stmt = mysqli_prepare($link, $query)) {
            mysqli_stmt_bind_param($stmt, "s", $twelveMonthsAgo);
    
            if (mysqli_stmt_execute($stmt)) {
                echo "Expired events deleted successfully.";
            } else {
                echo "Failed to delete expired events: " . mysqli_error($link);
            }
